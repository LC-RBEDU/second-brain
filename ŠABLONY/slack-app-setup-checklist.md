...
- `users:read`
- `users:read.email`
- `search:read` — `search.messages` (DM/mention/`hasmy::gear:` discover v `slack_poll.py`)
...
- `reactions:write` — **user** token: po zápisu do INBOX sundat tvoji `:gear:` (`reactions.remove`). Bot `reactions:write` (výše) = n8n ✅ na capture kanálu — to nestačí na unreact tvé reakce.
...
`slack_poll.py` na VPS je SSOT pro dohledávání nových zpráv v DM/GDM, @zmínkách a vláknech označených **`:gear:`** (ne Slack Later / `is:saved`). **Předtím pauzni oba** Cowork scheduled Slack archive tasky (sdílejí prompt s `on:dnešní datum`):
